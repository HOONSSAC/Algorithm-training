...
#define ENCODED_FILE_PATH "sample.enc"
...
struct MinHeapNode* buildHuffmanTree(char data[], int freq[], int size) {
    ...
    struct MinHeap* minHeap = createAndBuildMinHeap(data, freq, size); // 이 부분을 수정했습니다.
...
void HuffmanCodes(char data[], int freq[], int size, char codes[ALPHABET_SIZE][MAX_TREE_HT]) {
    ...
    printCodes(root, arr, top, codes); // 수정된 printCodes 호출
}

void encodeFile(char codes[ALPHABET_SIZE][MAX_TREE_HT]) {
    FILE* file = fopen(FILE_PATH, "r");
    FILE* encodedFile = fopen(ENCODED_FILE_PATH, "w");
